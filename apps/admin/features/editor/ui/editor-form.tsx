...
import CoverImageField from "@/components/editor/fields/cover-image-field";
...
type Props = {
  initialPost?: PostRecordCurrent | null;
  drafts?: DraftSummary[];
};
...
export function EditorForm({ initialPost, drafts = [] }: Props) {
  ...
  const [cover, setCover] = useState(initialPost?.coverImageUrl ?? "");
  const [content, setContent] = useState<PostContent>(
    initialPost?.content ?? blankDoc
  );
  ...
  const createdInfo = initialPost
    ? `${initialPost.status === "published" ? "Published" : "Drafted"} ${formatDate(initialPost.createdAt)}`
    : null;
  ...
  const handleSave = (nextStatus: PostStatus) => {
    ...
    startTransition(async () => {
      ...
      const targetSlug = result.data.slug ?? initialPost?.slug;
      const destination =
